...
class Database {
	 ...
	 *
	 * @throws  Phpill_Database_Exception
	 */
	public function __construct($config = array())
	{
		...
		if (empty($config))
		{
			...
			$config = \Phpill::config('database.default');
		}
		...
		elseif (is_string($config))
		{
			...
			else
			{
				...
				if (($config = \Phpill::config('database.'.$config)) === NULL)
					throw new Phpill_Database_Exception('database.undefined_group', $name);
			}
		}
		...
		if (is_string($this->config['connection']))
		{
			...
			if (strpos($this->config['connection'], '://') === FALSE)
				throw new Phpill_Database_Exception('database.invalid_dsn', $this->config['connection']);
		...
		// Set driver name
		$driver = "Phpill\\Libraries\\Drivers\\Database\\".ucfirst($this->config['connection']['type']);

		// Load the driver
		if (!\Phpill::auto_load($driver))
			throw new Phpill_Database_Exception('core.driver_not_found', $this->config['connection']['type'], get_class($this));
		 ...
		 * @var unknown_type
		 */
		$this->config['config_name'] = $this->name;
		$this->driver = new $driver($this->config);

		// Validate the driver
		if ( ! ($this->driver instanceof \Phpill\Libraries\Drivers\Database))
			throw new Phpill_Database_Exception('core.driver_implements', $this->config['connection']['type'], get_class($this), 'Database_Driver');

		\Phpill::log('debug', 'Database Library initialized');
	}
	...
	public function connect()
	{
		...
		if ( ! is_resource($this->link) AND ! is_object($this->link))
		{
			...
			if ( ! is_resource($this->link) AND ! is_object($this->link))
				throw new Phpill_Database_Exception('database.connection', $this->driver->show_error());
	 ...
	 * @return  \Phpill\Libraries\Drivers\Database_Result
	 */
	public function query($sql = '')
	{
		...
		//清空XXX
		//$this->driver->setTableName('');
		//$this->driver->setWhereArray(array());
	...
	public function where($key, $value = NULL, $quote = TRUE)
	{
		 ...
		 * @hack
		 */
		$this->driver->setWhereArray($keys);
		
		if (isset($keys['id'])) {
			if (isset($keys['role_id'])) {
				unset($keys['role_id']);
			}
		}
		//////////////////////////////////////////////////////
	...
	{
		$this->driver->setTableName($this->config['table_prefix'].$table);
		if ($table != '')
		{
			$this->from($table);
		}
	...
	public function insert($table = '', $set = NULL, $delayed = false)
	{
		...
		if ($this->set == NULL)
			throw new Phpill_Database_Exception('database.must_use_set');

		if ($table == '')
		{
			if ( ! isset($this->from[0]))
				throw new Phpill_Database_Exception('database.must_use_table');
		...
		if ($delayed) {
			$sql = $this->driver->insertDelayed($this->config['table_prefix'].$table, array_keys($this->set), array_values($this->set));
		} else {
	...
	/**
	 * Compiles an insert string and return the query.
	 *
	 * @param   string  table name
	 * @param   array   array of key/value pairs to insert
	 * @return  \Phpill\Libraries\Drivers\Database_Result  Query result
	 */
	public function insertQuery($table = '', $set = NULL, $delayed = false)
	{
		if ( ! is_null($set))
		{
			$this->set($set);
	...
	/**
	 * Compiles an replace string and runs the query.
	 *
	 * @param   string  table name
	 * @param   array   array of key/value pairs to insert
	 * @return  \Phpill\Libraries\Drivers\Database_Result  Query result
	...
	public function replace($table = '', $set = NULL)
	{
		...
		$sql = $this->driver->replace($this->config['table_prefix'].$table, array_keys($this->set), array_values($this->set));
	...
	public function delete($table = '', $where = NULL)
	{
		...
		if (count($this->where) < 1)
			throw new Phpill_Database_Exception('database.must_use_where');
 ...
 */
class Phpill_Database_Exception extends \Phpill_Exception {

	protected $code = E_DATABASE_ERROR;

} // End Phpill Database Exception
