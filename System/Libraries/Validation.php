...
class Validation extends ArrayObject {
	...
	protected function callback($callback)
	{
		...
		if ( ! is_callable($callback, FALSE))
		{
			...
			throw new Phpill_Exception('validation.not_callable', $name);
		}
	...
	public function errors($file = NULL)
	{
		...
		else
		{
			...
			foreach ($this->errors as $input => $error)
			{
				...
				if (($errors[$input] = Phpill::lang($key)) === $key)
				{
					// Get the default error message
					$errors[$input] = Phpill::lang("$file.$input.default");
				}
			}
	 ...
	 * 数字最小取值
	 */
	public function minnumlength($num, $min)
	{
		if ( ! is_numeric($num))
			return FALSE;
		$status = FALSE;
