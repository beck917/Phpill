 ...
 */
namespace System\Libraries\Drivers\Database;
class Mysqli extends \System\Libraries\Drivers\Database\Mysql {
	...
	public function __construct($config)
	{
		...
		\Phpill::log('debug', 'MySQLi Database Driver Initialized');
	}
	...
	public function query($sql)
	{
		...
		if ($this->db_config['cache'] AND ! preg_match('#\b(?:INSERT|UPDATE|REPLACE|SET)\b#i', $sql))
		{
			...
			if ( ! isset(self::$query_cache[$hash]))
			{
				...
				self::$query_cache[$hash] = new Phpill_Mysqli_Result($this->link, $this->db_config['object'], $sql);
			}
		...
		return new Phpill_Mysqli_Result($this->link, $this->db_config['object'], $sql);
	}

	public function set_charset($charset)
	{
		if ($this->link->set_charset($charset) === FALSE)
			throw new \System\Libraries\Phpill_Database_Exception('database.error', $this->show_error());
	}

	public function stmt_prepare($sql = '')
	{
		is_object($this->link) or $this->connect();
		return new Phpill_Mysqli_Statement($sql, $this->link);
	}
 ...
 * MySQLi Result
 */
class Phpill_Mysqli_Result extends \System\Libraries\Drivers\Database_Result {
	...
	public function __construct($link, $object = TRUE, $sql)
	{
		...
		{
			// SQL error
			throw new \System\Libraries\Phpill_Database_Exception('database.error', $this->link->error.' - '.$sql);
		}
		else
		{
	...
	public function result($object = TRUE, $type = MYSQLI_ASSOC)
	{
		...
		{
			$this->return_type = (is_string($type) AND \Phpill::auto_load($type)) ? $type : 'stdClass';
		}
		else
		{
	...
	public function result_array($object = NULL, $type = MYSQLI_ASSOC)
	{
		...
		elseif (is_bool($object))
		{
			if ($object === TRUE)
			{
				...
				// NOTE - The class set by $type must be defined before fetching the result,
				// autoloading is disabled to save a lot of stupid overhead.
				$type = (is_string($type) AND \Phpill::auto_load($type)) ? $type : 'stdClass';
			}
			else
			{
				$fetch = 'fetch_array';
