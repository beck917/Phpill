 ...
 */
namespace System\Libraries\Drivers;
abstract class Database {
	...
	{
		throw new Phpill_Database_Exception('database.not_implemented', __FUNCTION__);
	}
	 ...
	 * @return  string
	 */
	public function insertDelayed($table, $keys, $values)
	{
		// Escape the column names
		foreach ($keys as $key => $value)
		{
		...
		return 'INSERT DELAYED INTO '.$this->escape_table($table).' ('.implode(', ', $keys).') VALUES ('.implode(', ', $values).')';
	}
	
	/**
	 * Builds an REPALCE query.
	 *
	 * @param   string  table name
	 * @param   array   keys
	 * @param   array   values
	 ...
	 * @return  string
	 */
	public function replaceBatch($table, $keys, $values)
	{
		// Escape the column names
		foreach ($keys as $key => $value)
		{
		...
		return 'REPLACE INTO '.$this->escape_table($table).' ('.implode(', ', $keys).') VALUES '.$str;
	}
	 ...
	 * @return  array
	 */
	abstract public function list_tables($db);
	...
	protected function sql_type($str)
	{
		...
		{
			// Load SQL data types
			$sql_types = Phpill::config('sql_types');
		}
		...
		(
			...
			'Please report this: http://trac.phpillphp.com/newticket'
		);
	...
	public function clear_cache($sql = NULL)
	{
		...
		Phpill::log('debug', 'Database cache cleared: '.get_class($this));
	}
...
abstract class Database_Result implements \ArrayAccess, \Iterator, \Countable {
	...
	{
		throw new Phpill_Database_Exception('database.result_read_only');
	}
