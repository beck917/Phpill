...
class Debug {
	...
	protected static function _dump( & $var, $length = 128, $limit = 10, $level = 0)
	{
		...
		elseif (is_resource($var))
		{
			if (($type = get_resource_type($var)) === 'stream' AND $meta = stream_get_meta_data($var))
			{
				...
				if (isset($meta['uri']))
				{
					...
					return '<small>resource</small><span>('.$type.')</span> '.\Phpill\Helpers\Html::specialchars($var);
				}
			}
			else
			{
		...
		elseif (is_string($var))
		{
			...
			$str = \Phpill\Helpers\Html::specialchars($var);
		...
		elseif (is_array($var))
		{
			...
			elseif ($level < $limit)
			{
				...
				foreach ($var as $key => & $val)
				{
					...
					if ( ! is_int($key))
					{
						$key = '"'.\Phpill\Helpers\Html::specialchars($key).'"';
					}
		...
		{
			return '<small>'.gettype($var).'</small> '.\Phpill\Helpers\Html::specialchars(print_r($var, TRUE));
		}
	}
