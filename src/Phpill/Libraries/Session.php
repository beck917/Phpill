...
class Session {
	...
	public function __construct()
	{
		...
		{
			// Load config
			self::$config = \Phpill::config('session');
		...
		\Phpill::log('debug', 'Session Library initialized');
	}
	...
	public function create($vars = NULL)
	{
		...
		{
			// Set driver name
			$driver = $driver = "Phpill\\Libraries\\Drivers\\Session\\".ucfirst(self::$config['driver']);
		...
		(
			...
			\Phpill::config('cookie.path'),
			\Phpill::config('cookie.domain'),
			\Phpill::config('cookie.secure'),
			\Phpill::config('cookie.httponly')
		);
		...
		if ( ! isset($_SESSION['_kf_flash_']))
		{
			...
			$_SESSION['user_agent'] = \Phpill::$user_agent;
		...
		if ($_SESSION['total_hits'] > 1)
		{
			...
			foreach (self::$config['validate'] as $valid)
			{
				switch ($valid)
				{
					...
					case 'user_agent':
						if ($_SESSION[$valid] !== \Phpill::$user_agent)
							return $this->create();
					break;
	...
	{
		//XXXsession_id在session_start之前为空
		if (session_id() !== '')
		{
			// Get the session name
			$name = session_name();
	...
	public function get($key = FALSE, $default = FALSE)
	{
		...
		$result = isset($_SESSION[$key]) ? $_SESSION[$key] : \Phpill::key_string($_SESSION, $key);
