...
class Router {
	...
	/**
	 * Router setup routine. Automatically called during Phpill setup process.
	 *
	 * @return  void
	 */
	public static function setup()
	{
		...
		{
			// Load routes
			self::$routes = \Phpill::config('routes');
		}
		...
		if (self::$current_uri === '')
		{
			...
			if ( ! isset(self::$routes['_default']))
				throw new Phpill_Exception('core.no_default_route');
		...
		//首字母大写
		self::$current_uri = ucfirst(self::$current_uri);

		// Make sure the URL is not tainted with HTML characters
		self::$current_uri = \Phpill\Helpers\Html::specialchars(self::$current_uri, FALSE);
	...
	public static function find_uri()
	{
		...
		elseif (isset($_GET['phpill_uri']))
		{
			// Use the URI defined in the query string
			self::$current_uri = $_GET['phpill_uri'];

			// Remove the URI from $_GET
			unset($_GET['phpill_uri']);

			// Remove the URI from $_SERVER['QUERY_STRING']
			$_SERVER['QUERY_STRING'] = preg_replace('~\bphpill_uri\b[^&]*+&?~', '', $_SERVER['QUERY_STRING']);
		}
		...
		if (($strpos_fc = strpos(self::$current_uri, $fc)) !== FALSE)
		{
			...
			//Modify @2014/5/19 /init/ compatible
			self::$current_uri = substr(self::$current_uri, 0, $strpos_fc);
		}
		...
		if (self::$current_uri !== '')
		{
			if ($suffix = \Phpill::config('core.url_suffix') AND strpos(self::$current_uri, $suffix) !== FALSE)
			{
