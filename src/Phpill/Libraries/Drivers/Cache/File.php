...
class File extends \Phpill\Libraries\Drivers\Cache {
	...
	public function __construct($directory)
	{
		...
		if ( ! is_dir($directory) OR ! is_writable($directory))
			throw new \Phpill_Exception('cache.unwritable', $directory);
	...
	public function delete($id, $tag = FALSE)
	{
		...
		foreach ($files as $file)
		{
			...
			if ( ! unlink($file))
				Phpill::log('error', 'Cache: Unable to delete cache file: '.$file);
		}
		...
		if ($files = $this->exists(TRUE))
		{
			foreach ($files as $file)
			{
				if ($this->expired($file))
				{
					...
					@unlink($file) or Phpill::log('error', 'Cache: Unable to delete cache file: '.$file);
				}
			}
		}
	}
