...
class Encrypt {
	...
	public function __construct($config = FALSE)
	{
		if ( ! defined('MCRYPT_ENCRYPT'))
			throw new Phpill_Exception('encrypt.requires_mcrypt');
		...
		if (is_string($config))
		{
			...
			if (($config = Phpill::config('encryption.'.$config)) === NULL)
				throw new Phpill_Exception('encrypt.undefined_group', $name);
		}

		if (is_array($config))
		{
			// Append the default configuration options
			$config += Phpill::config('encryption.default');
		}
		else
		{
		...
		if (empty($config['key']))
			throw new Phpill_Exception('encrypt.no_encryption_key');
		...
		Phpill::log('debug', 'Encrypt Library initialized');
	}
